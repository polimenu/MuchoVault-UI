...
export const EditCorpModal = ({ uuid }: { uuid: string }) => {
    ...
    useEffect(() => {
        if (corpDetails) {
            setLegalName(corpDetails.legal_name);
            setContactName(corpDetails.contact_details.name);
            setContactEmail(corpDetails.contact_details.email);
            setContactPhone(corpDetails.contact_details.phone);
            setRegistrationNumber(corpDetails.registration_number);
            setPC(corpDetails.registered_address.post_code);
            setCity(corpDetails.registered_address.city);
            //setCountry
            setAddr1(corpDetails.registered_address.address_line_1);
            setAddr2(corpDetails.registered_address.address_line_2);
            setTargetAddress(corpDetails.target_address);
            setCorpType(corpDetails.type);
        }
    }, [corpDetails])

    const { state } = useGlobal();
    const [isDone] = useEditCorp(uuid, rampState.sessionId, req);
