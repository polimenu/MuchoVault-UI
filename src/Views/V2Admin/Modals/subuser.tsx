...
const Subscribe = ({ planId, head, unit, tokenContract, call, precision, decimals, amount, button }:
  ...
  const { approve } = useGetApprovalAmount(
    ...
    V2ADMIN_CONFIG[activeChain.id].MuchoBadgeManager
  );
  ...
  const allowance = useGetAllowance(tokenContract.contract, decimals, V2ADMIN_CONFIG[activeChain.id]?.MuchoBadgeManager, activeChain.id);
